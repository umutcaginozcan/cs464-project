...
%  Cost-Sensitive and Interpretable Stroke Risk Prediction
% ============================================================
...
\fancypagestyle{main}{%
  \fancyhf{}
  \fancyhead[L]{\small\itshape Stroke Risk Prediction on Tabular Clinical Data}
...
The dataset is divided into an 80\% training set and a 20\% held-out test set using \emph{stratified} random sampling (random seed 42) to preserve the class ratio in both splits.
...
Oversampling (SMOTE, ADASYN) is applied \emph{after} the preprocessor and \emph{only} on the training set within each cross-validation fold to prevent information leakage into the validation fold.
Similarly, class-weight arguments are passed directly to model constructors.

\subsubsection{Cross-Validation}

All hyperparameter decisions and model comparisons use 5-fold Stratified Cross-Validation on the training set, which preserves the minority-class ratio across folds.
...
\begin{table}[H]
  \centering
  \caption{Tools, libraries, and frameworks used.}
  ...
  \begin{tabular}{lll}
    ...
    ML framework   & scikit-learn         & Classical models, pipelines, evaluation \\
    Imbalance      & imbalanced-learn     & SMOTE, ADASYN, BorderlineSMOTE \\
    Boosting       & XGBoost              & Gradient-boosted trees \\
    Deep learning  & PyTorch              & MLP implementation \\
    HPO            & Optuna               & Bayesian hyperparameter optimisation \\
    Explainability & SHAP                 & Global and local feature attribution \\
    Explainability & LIME                 & Local surrogate explanations \\
    Visualisation  & matplotlib, seaborn  & Figures and plots \\
    Environment    & Conda / pip          & Package management \\
    Version ctrl   & Git                  & Code versioning \\
    \bottomrule
  \end{tabular}
\end{table}
...
We apply two post-hoc calibration methods to the winner model:
\begin{itemize}
  \item \textbf{Platt scaling} — fits a logistic regression on the model's raw scores.
  \item \textbf{Isotonic regression} — a non-parametric monotone function fitted to the scores.
\end{itemize}
...
F.~Soriano.
\newblock Stroke prediction dataset.
\newblock Kaggle, 2021.
